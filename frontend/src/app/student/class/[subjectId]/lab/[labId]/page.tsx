...
export default function StudentsLabPage() {
  ...
  const handleStartEvaluation = () => {
    ...
    // You might want to update lab.evaluationStatus to 'in-progress' here in your global state/backend
    // onUpdateLabEvaluationStatus(lab.id, 'in-progress'); // Example of pre-marking
  };
  ...
  const handleModalClose = (
    ...
    // Update the lab's evaluation status in your backend/global state
  };
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-lg p-8 space-y-6">
          ...
          {lab.submitted ? (
            <div className="bg-green-50/50 border border-green-200 rounded-lg p-4 flex items-center gap-4">
              ...
              <p className="text-gray-700 font-medium">
                Submitted{" "}
                {lab.submissionFile ? `(${lab.submissionFile.name})` : ""}
                {lab.evaluated ? (
        ...
        {lab.submitted &&
          !lab.evaluated  && ( // Only show if submitted and not evaluated yet
            <Button
